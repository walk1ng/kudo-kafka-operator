	...
	Describe("[Kafka TLS]", func() {
		Context("tls enabled", func() {
			...
			It("verify the certs", func() {
				...
				crtMD5, err := kafkaClient.ExecInPod(customNamespace, "kafka-kafka-0", suites.DefaultContainerName,
					[]string{"openssl", "x509", "-noout", "-modulus", "-in", "/etc/tls/certs/tls.crt", "|", "openssl", "md5"})
				Expect(err).To(BeNil())
				keyMD5, err := kafkaClient.ExecInPod(customNamespace, "kafka-kafka-0", suites.DefaultContainerName,
